...
nothing about bundles. It is found beside this script first - AppletBuilder.app vendors
the whole set into Contents/Library/python_thinning, so its Build & Run pane and
`appletbuilder thin-python` need no checkout - and otherwise in a Python-Embedding
checkout sitting next to the OMC repo.
...
restores the old run-everything behavior; measured on one applet scanning keeps 40.3 MB
...
names. Each such tool's directory is analyzed exactly as Resources/Scripts is, so the
...
# Every subprocess below runs an applet's own interpreter over that applet's own
# stdlib, and CPython caches bytecode next to the source it imports - inside the
# bundle. Children only: this process's own imports are already done by the time
# this line runs, and it is not the one importing out of an applet. That dirties a tree the caller is about to sign, measure, or sweep
# (AppletBuilder's build removes __pycache__, and its test suite fails on one it
# did not put there). OMC's handlers already export a prefix; a developer running
# this by hand does not, which is the route this tool's own error messages suggest.
os.environ.setdefault(
    "PYTHONPYCACHEPREFIX",
    os.path.join(tempfile.gettempdir(), "thin_applet_python_pyc"))

HERE = os.path.dirname(os.path.abspath(__file__))
OMC_ROOT = os.path.dirname(os.path.dirname(HERE))
PE = os.path.join(os.path.dirname(OMC_ROOT), "Python-Embedding")

# The generic toolkit this front end drives. thin_with_plan.sh runs
# thin_python_distribution.sh from its own directory, so all three travel together.
TOOLKIT_FILES = ("analyze_python_deps.py", "thin_with_plan.sh",
                 "thin_python_distribution.sh")


def toolkit_dir():
    """Where the generic toolkit lives.

    Beside this script first: that is the copy vendored into AppletBuilder.app
    (Contents/Library/python_thinning), which is what lets the Build & Run pane
    and `appletbuilder thin-python` work on a machine with no Python-Embedding
    checkout at all. Otherwise the sibling checkout next to the OMC repo, which is
    where the repo's own copy of this script has always found it.

    A location has to provide the WHOLE set to be chosen. Half a toolkit is a
    broken install, and taking two files from one place and the third from the
    other would run an analyzer from one version against an applier from another.
    """
    for cand in (HERE, PE):
        if all(os.path.exists(os.path.join(cand, name)) for name in TOOLKIT_FILES):
            return cand
    return PE          # nothing complete; main() names what is missing and where


TOOLKIT = toolkit_dir()
ANALYZER = os.path.join(TOOLKIT, "analyze_python_deps.py")
APPLY = os.path.join(TOOLKIT, "thin_with_plan.sh")
...
def analyzer_driver(pybin):
    ...
    # -I: the question is whether THIS interpreter still has these modules, so nothing
    # outside it may answer. It implies -E and -s (no PYTHON* environment, no user
    # site) and also drops the "" entry -c puts on sys.path - without that last part a
    # datetime.py in whatever directory the caller happened to be in decides the
    # answer, in both directions. -B because -I drops the cache prefix set above, and
    # the probe must not write bytecode into the distribution it is inspecting.
    probe = subprocess.run([pybin, "-B", "-I", "-c",
                            "import json, ast, datetime, argparse"],
                           capture_output=True)
...
def cmd_plan(args):
    ...
    try:
        ...
        if os.path.isdir(cl.scripts):
            ...
            print("  (no %s - nothing to analyze)" % REL_SCRIPTS)
...
def main(argv):
    ...
    # Against the directory that was actually chosen. TOOLKIT falls back to PE when
    # NEITHER location is complete, so listing what is missing from PE described a
    # place the reader was not told had been searched - and called all three files
    # missing when the vendored copy was short only one.
    beside = [n for n in TOOLKIT_FILES if not os.path.exists(os.path.join(HERE, n))]
    missing = [n for n in TOOLKIT_FILES if not os.path.exists(os.path.join(TOOLKIT, n))]
    if missing and TOOLKIT != HERE and len(beside) < len(missing):
        missing = beside
    if missing:
        # PE is derived from this file's location, so for the copy vendored inside
        # AppletBuilder.app it names a directory that was never going to exist -
        # <App>.app/Python-Embedding. Testing its parent said "yes" there, because
        # the parent is the .app itself, and the reader was sent after it anyway.
        # The repository copy is the only layout where a sibling checkout is real.
        where = "Looked beside this script:\n    %s" % HERE
        in_bundle = ".app%sContents%s" % (os.sep, os.sep) in HERE + os.sep
        if not in_bundle and os.path.isdir(os.path.dirname(PE)):
            where += "\n  and in a sibling Python-Embedding checkout:\n    %s" % PE
        fail("the Python-Embedding toolkit is incomplete - missing %s\n"
             "%s\n"
             "Fetch it as a sibling of OMC: https://github.com/abra-code/Python-Embedding\n"
             "AppletBuilder.app carries its own copy; update_appletbuilder.sh refreshes it."
             % (", ".join(missing), where))
    return args.fn(args)
